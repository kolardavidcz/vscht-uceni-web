...
- `pnpm dev` does **not** run Vercel serverless functions.
...
- Build: `pnpm build`, output directory `dist`
...
| Expect `pnpm dev` to run all Vercel APIs | Only suggest-edit is local-middleware’d |
...
| Use `npm` or `yarn` or call `node_modules/.bin` binaries directly | Always use `pnpm` |
