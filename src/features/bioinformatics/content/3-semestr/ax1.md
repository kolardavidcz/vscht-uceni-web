# 💻 BI-AX1 (Algoritmy a grafy)

<div class="my-6 p-4 rounded-2xl border border-stone-200 dark:border-stone-800 bg-stone-50 dark:bg-stone-900/60 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
  <div>
    <div class="font-bold text-stone-900 dark:text-stone-100 text-sm">Letní příprava na BI-AX1 (Pre-AX1)</div>
    <div class="text-xs text-stone-600 dark:text-stone-400">Kompletní matematický základ, grafová teorie i kódovací C++ taháky.</div>
  </div>
  <a href="/obor-bioinformatika/3-semestr/pre-ax1/math/dml" class="wiki-btn shrink-0">
    <span>Letní příprava Pre-AX1</span>
    <span>→</span>
  </a>
</div>

## 📝 Průběh cvičení a hodnocení (AG1)

* 🧠 **Kvízy ze cvičení probíhají v Marastu**: Průběžné cvičební testy a kvízy se píší v testovacím prostředí **[Marast FIT](https://marast.fit.cvut.cz/)**. Doporučujeme si předem projít tamní cvičné úlohy na grafy a algoritmy.
* 📊 **Zapisování bodů a známek do Grades**: Získané body z kvízů i celkové průběžné hodnocení se nově zapisují do systému **[Grades FIT](https://grades.fit.cvut.cz/)**.

## 📚 Užitečné odkazy a systémy pro studium AX1:

* 📝 **[Studentská skripta](https://truepeak78.github.io/ax1/)**  
---
* 📐 **[Procvičení před cvičením z AX1](https://courses.fit.cvut.cz/BI-AX1/prerekvizity/index.html)**
* 🌐 **[Graph Online](https://graphonline.top/en/)** - převod ze schématu grafu na text
---
* 📑 **[Anki kartičky](https://gitlab.com/xskoda/fit-cvut-anki)** - na procvičování pojmů, konceptů a definic
* 📂 **[Cvičné testy a marast příklady (Google Disk)](https://drive.google.com/drive/folders/1UPdKA_3CejwKSNMGtKcZRPUsSdbZHYUH)** nebo přímý **[fit-wiki .rar archiv](https://fit-wiki.cz/_media/%C5%A1kola/p%C5%99edm%C4%9Bty/ax1_selftest.rar)**
