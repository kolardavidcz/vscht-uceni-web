# 🧭 Kontakty, systémy & rozcestník pro celé studium

Tato stránka slouží jako **univerzální rozcestník a záchranná síť** pro studenty bioinformatiky po celou dobu bakalářského i navazujícího studia. Najdete zde oficiální kontakty, přístupy do školních systémů, tipy na studijní literaturu i praktické nástroje pro každodenní studium.

---

## 👥 Důležité kontakty a záchranná síť

### VŠCHT Praha (Ústav informatiky a chemie — ÚICH)
* **Garant bakalářského programu**:  
  **Ing. Martin Šícho, Ph.D.** — `Martin.Sicho@vscht.cz`  
  *(Kontaktujte při jakýchkoliv systémových potížích, dotazech k organizaci studia či uznávání předmětů.)*
* **Garant magisterského a Ph.D. programu**:  
  **prof. Daniel Svozil, Ph.D.** — `Daniel.Svozil@vscht.cz`  
  *(Koordinátor česko-francouzského double-degree programu VŠCHT-Štrasburk.)*
* **Tajemník ústavu (rozvrh a organizace)**:  
  **Ing. Jiří Znamenáček** — `Jiri.Znamenacek@vscht.cz`
* **Děkanát FCHT VŠCHT (studijní referentky)**:  
  * Bakalářské studium: **Petra Kohoutová** — tel. `+420 22044 4148`, `Petra.Kohoutova@vscht.cz`  
  * Magisterské studium: **Klára Voltrová** — tel. `+420 22044 3767`, `Klara.Voltrova@vscht.cz`
* **Pedagogické oddělení VŠCHT (celoškolní agenda)**:  
  **Ing. Petra Kubová** — tel. `+420 22044 4139`, `Petra1.Kubova@vscht.cz`

### FIT ČVUT (Smluvní výuka)
* **Centrální e-mail pro výuku bioinformatiky na FIT**: `bioinformatika@fit.cvut.cz`  
  *(E-maily putují přímo pověřeným pracovníkům studijního oddělení FIT ČVUT.)*
* **Koordinátorka studijní agendy bioinformatiky na FIT**: **Ing. Regina Šmídová**
* **Studijní oddělení FIT ČVUT**: **Zdeňka Kutinová**, **Ivana Dolejšová** ([kontakty na studijní oddělení FIT](https://fit.cvut.cz/cs/studium/informacni-servis/studijni-oddeleni))

---

## ☕ Komunita, zázemí & studovna B1322

> 💡 **Zkušenost z praxe katedry**:  
> Pravidelní účastníci komunitního života ústavu mají statisticky **výrazně vyšší úspěšnost dokončení studia**. Sdílení know-how se staršími spolužáky i vyučujícími v neformálním prostředí je nejrychlejší cesta k překonání krizových momentů.

* **Discord BioCord**:  
  Oficiální komunitní server studentů a vyučujících oboru: **[discord.gg/yWxFJmM6Qg](https://discord.gg/yWxFJmM6Qg)**.  
  ⚠️ **Povinnost:** Nastavte si svou přezdívku na serveru na civilní **Jméno Příjmení** — slouží jako primární oficiální kanál ÚICH.
* **Klubovna a volná studovna B1322 (budova B VŠCHT)**:  
  Seminární místnost B1322 ([mapa ÚICH](https://uich.vscht.cz/mapa)) slouží mimo rozvrhovanou výuku jako otevřená studovna a klubovna pro bioinformatiky. Můžete zde trávit volné chvíle mezi přednáškami, učit se a programovat.
* **Orientace v dejvickém kampusu**:
  * Interaktivní mapy budov VŠCHT: [emil.vscht.cz/maps](https://emil.vscht.cz/maps)
  * Plánky učeben a navigace FIT ČVUT (Nová budova ČVUT & THÁKUROVA): [help.fit.cvut.cz](https://help.fit.cvut.cz)

---

## 🚀 Rychlý rozcestník informačních systémů

* **Informační systémy VŠCHT**:  
  * [SIS VŠCHT](https://sis.vscht.cz/) — rozvrhy, zápisy termínů zkoušek, index a oficiální studijní výsledky.
  * [Moodle VŠCHT](https://e-learning.vscht.cz/) — studijní opory, e-learningové testy a materiály k předmětům VŠCHT.
* **Informační systémy ČVUT**:  
  * [KOS ČVUT](https://kos.cvut.cz) — studijní systém FIT pro cvičení a zkoušky z informatiky.
  * [Courses FIT](https://courses.fit.cvut.cz) — výukový portál předmětů FIT (slajdy, domácí úlohy, zadání).
  * [Usermap ČVUT](https://usermap.cvut.cz) — správa hesel a profilu uživatele ČVUT.
* **Wi-Fi připojení (eduroam)**:  
  Pro spolehlivé připojení k celouniverzitní síti eduroam si stáhněte bezpečnostní certifikát z [webu Výpočetního centra VŠCHT](https://vc.vscht.cz/navody/sit/wi-fi).
* **Tisk v Národní technické knihovně (NTK)**:  
  Nejspolehlivější tisk z vlastního notebooku na tiskárnách NTK zajistíte instalací [speciálního ovladače tiskáren NTK](https://www.techlib.cz/cs/2912-jak-nainstalovat-ntk-tiskarny-na-vlastni-zarizeni).

---

## 📚 Studijní literatura & akademická komunikace

* **Skripta a literatura**:  
  Mnoho oficiálních skript je studentům VŠCHT dostupných online zdarma ve [Vydavatelství VŠCHT](https://vydavatelstvi.vscht.cz/) nebo ve [starším archivu e-learningu](https://e-learning.vscht.cz/course/view.php?id=1076).
* **Oslovování vyučujících (Etiketa)**:  
  Při e-mailové i osobní komunikaci vždy oslovujeme **nejvyšším dosaženým titulem** (např. *Vážený pane profesore*, *Vážený pane docente*, nikoliv *Vážený pane inženýre*). Pokud si nejste jistí pořadím titulů, ověřte si je v aplikaci [Titulátor](https://titulator.smid.io/).

---

## 📝 Jak efektivně dělat zápisky z přednášek

* **Offline přednášky (v posluchárnách)**:  
  * **Psaní rukou vs. klávesnice:** Poznámky psané rukou (pero + sešit nebo tablet se stylusem) prokazatelně zlepšují zapamatování — nutí vás látku přeformulovat a zhušťovat.
  * **Double-screen strategie:** Mějte prezentaci otevřenou na mobilu/tabletu a v sešitě pište jen doplňující komentáře vyučujícího, které na slajdech nejsou.
  * **Tisk klíčových přehledů:** Vytiskněte si periodickou tabulku, aminokyseliny a základní vzorce na papír, abyste při psaní poznámek nemuseli neustále přepínat okna.
* **Online přednášky a videozáznamy**:  
  * **Přeskakování ticha:** Při sledování přednášek z YouTube/Vimeo použijte rozšíření prohlížeče [Jump Cutter](https://chromewebstore.google.com/detail/jump-cutter/lmppdpldfpfdlipofacekcfleacbbncp), které automaticky přeskakuje tiché pasáže a ušetří 15–20 % času.
* **Kód v zápiscích**:  
  Pokud tvoříte poznámky v Microsoft Wordu, pro vkládání obarveného zdrojového kódu vyzkoušejte doplněk [Easy Syntax Highlighter](https://appsource.microsoft.com/en-us/product/office/wa200000011?tab=overview) (doporučené téma: *A11y Light* nebo *Tomorrow Night*).

---

## 🌲 Pravidelné akce oboru

* **Bioinformatické vítání prváků (listopad)**:  
  Pravidelný celodenní pěší výlet studentů všech ročníků a vyučujících ÚICH do přírody první listopadovou sobotu. Ideální příležitost poznat starší spolužáky i vyučující.
* **SVK — Studentská vědecká konference (konec listopadu)**:  
  Celoškolní den studentské vědy spojený s rektorským volnem. Sekce bioinformatiky probíhá v angličtině a představuje skvělou inspiraci pro volbu tématu vlastní bakalářské práce.
* **BIOPUCT**:  
  Výroční konference doktorandů oboru Bioinformatika s prezentacemi aktuálních výzkumných projektů.
* **Double-degree VŠCHT–Štrasburk**:  
  Prestižní navazující magisterský program umožňující získat francouzský i český diplom.
